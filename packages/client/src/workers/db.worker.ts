...
import { Constants, Utils } from '@game3js/common'
...
function queueCall (func) {
  ...
  this._promiseQueue =
    this._promiseQueue ||
    Utils.promiseQueue(this._callBuffer, () => {
...
async function startIPFS () {
  ipfs = await Utils.IPFSCreate(options.ipfs);
  return ipfs;
}

async function startOrbit (options) {
  this.orbit = await Utils.OrbitCreate(this.ipfs, options.orbit);
...
async function handleIpfsGetFile ({ options }) {
  try {
    const array: any = await getFile(this.orbit, options.hash, this.postMessage, options)
...
function getFile (orbit, hash, postMessage, options:any = {}) {
  return new Promise((resolve, reject) => {
    ...
    stream.on('data', chunk => {
      clearTimeout(timeoutTimer)
      array = Utils.concatUint8Arrays(array, chunk)
